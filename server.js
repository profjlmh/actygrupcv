...
lti.onConnect(async (token, req, res) => {
  ...
  try {
    const enrollmentsRes = await axios.get(
      `${PLATFORM_URL}/api/v1/courses/${courseId}/enrollments`,
      {
        headers: {
    ...
    console.log("Enrollments detectados:", enrollments.map(e => ({
      type: e.type,
      role: e.role,
      role_id: e.role_id,
      user_id: e.user_id
    ...
    const isTeacher = enrollments.some(e =>
    ...
    console.error("Error al validar rol en curso:", e.response?.data || e.message);
  }
